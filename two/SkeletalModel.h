...
public:
  ...
  void drawJointHierarchy(Joint* j, Matrix4f transform);
  void drawSkeletonHierarchy(Joint* j, Matrix4f transform);
  void computeBindWorldToJointTransformsHelper(Joint* j, Matrix4f transform);
  void updateCurrentJointToWorldTransformsHelper(Joint* j, Matrix4f transform);
  Vector3f interpolateHelper(Vector3f& v, std::vector<float>& w);
private:
